...
static void cdot(int nst, double *e, double **dv, double **soc, double complex *c, double complex *c_dot){
    ...
    for(ist = 0; ist < nst; ist++){
        ...
        for(jst = 0; jst < nst; jst++){
            if(ist != jst){
                ...
                so_term[ist] -= 1.0 * I * soc[ist][jst] * c[jst];
            }
        }
    }
    ...
    egs = e[0];
    for(ist = 0; ist < nst; ist++){
        c_dot[ist] = - 1.0 * I * c[ist] * (e[ist] - egs) + na_term[ist] + so_term[ist];
    }
...
static void rhodot(int nst, double *e, double **dv, double **soc, double complex **rho, double complex **rho_dot){
    ...
    for(ist = 0; ist < nst; ist++){
        for(jst = ist + 1; jst < nst; jst++){
            ...
            for(kst = 0; kst < nst; kst++){
                ...
                rho_dot[ist][jst] -= 1.0 * I * (soc[jst][kst] * rho[ist][kst] - soc[kst][ist] * rho[kst][jst]);
            }
