...
class Auxiliary_Molecule(object):
    ...
    def __init__(self, molecule):
        ...
        self.pos_old = np.zeros((molecule.nst, self.nat, self.ndim))
...
class SHXFv2(MQC):
    ...
    def __init__(self, molecule, thermostat=None, istate=0, dt=0.5, nsteps=1000, nesteps=20, \
        elec_object="coefficient", propagator="rk4", l_print_dm=True, l_adj_nac=True, hop_rescale="augment", \
        hop_reject="reverse", rho_threshold=0.01, sigma=None, init_coef=None, \
        l_econs_state=True, aux_econs_viol="fix", l_asymp=False, x_fin = 25.0, l_no_hop=False, \
        l_crunch = True, t_pc=0,\
        l_exact_pc=True, l_pc_w_phase_term=False, l_divide_rho=False, l_pc_nac=False, l_pc_rescale=False, \
...
#                raise ValueError (f"( {self.md_type}.{call_name()} ) {error_message} ( {error_vars} )")
        ...
        self.x_fin = x_fin
        self.l_no_hop = l_no_hop
        self.l_exact_pc = l_exact_pc
        self.l_pc_w_phase_term = l_pc_w_phase_term
        self.l_divide_rho = l_divide_rho
        self.l_pc_nac = l_pc_nac
        self.l_pc_rescale = l_pc_rescale
        ...
        self.phase_old = np.zeros((self.mol.nst, self.aux.nat, self.aux.ndim))
        self.dS = np.zeros((self.mol.nst, self.aux.nat, self.aux.ndim))
        self.dS_old = np.zeros((self.mol.nst, self.aux.nat, self.aux.ndim))
        self.nac_phase = np.zeros((self.aux.nat, self.aux.ndim))
        self.nac_phase_old = np.zeros((self.aux.nat, self.aux.ndim))
        self.alpha = np.ones(self.mol.nst)
        self.alpha_old = np.ones(self.mol.nst)
    ...
    def set_decoherence(self, one_st):
        ...
        """
        # Vectorized assignment for all states
        mol_pos = self.mol.pos[0:self.aux.nat, :]
        mol_vel = self.mol.vel[0:self.aux.nat, :]
        self.aux.pos[:, :, :] = mol_pos[np.newaxis, :, :]
        self.aux.pos_old[:, :, :] = mol_pos[np.newaxis, :, :]
        self.aux.vel[:, :, :] = mol_vel[np.newaxis, :, :]
        self.aux.vel_old[:, :, :] = mol_vel[np.newaxis, :, :]
        ...
        for ist in range(self.mol.nst):
            if (self.l_coh[ist]):
                if (self.l_first[ist]):
                    ...
                    self.aux.pos_old[ist] = self.mol.pos[0:self.aux.nat]
                else:
                    self.aux.pos_old[ist, :, :] = self.aux.pos[ist, :, :]
                    if (ist == self.rstate):
                        self.aux.pos[ist] = self.mol.pos[0:self.aux.nat]
                    else:
    ...
    def get_dS(self):
        ...
        if(self.l_pc_w_phase_term):
            self.dS_old[:, :, :] = self.phase_old[:, :, :]
            self.dS[:, :, :] = self.phase[:, :, :]
        else:
            # Vectorized: mass shape (nat,) -> broadcast to (nst, nat, ndim)
            self.dS_old = self.aux.vel_old * self.mol.mass[np.newaxis, :self.aux.nat, np.newaxis]
            self.dS = self.aux.vel * self.mol.mass[np.newaxis, :self.aux.nat, np.newaxis]

    def get_nac_phase(self):

        nat = self.aux.nat

        # Vectorized nac_phase calculation
        # nac: (nst, nst, nat, ndim), rho: (nst, nst) -> need to compute sum over ist, jst
        # Result: (nat, ndim)
        nac_rho = self.mol.nac[:, :, :nat, :] * self.mol.rho[:, :, np.newaxis, np.newaxis]  # (nst, nst, nat, ndim)
        nac_rho_old = self.mol.nac_old[:, :, :nat, :] * self.mol.rho[:, :, np.newaxis, np.newaxis]  # (nst, nst, nat, ndim)

        # Sum over ist, jst and divide by mass
        self.nac_phase = np.sum(np.imag(nac_rho), axis=(0, 1)) / self.mol.mass[:nat, np.newaxis]
        self.nac_phase_old = np.sum(np.imag(nac_rho_old), axis=(0, 1)) / self.mol.mass[:nat, np.newaxis]
            ...
            for ist in range(self.mol.nst):
                for jst in range(ist+1, self.mol.nst):
                    tmp = f'{self.aux.nat:6d}\n{"":2s}Step:{istep + 1:6d}{"":12s}Momentum (au)' + \
                        ...
                        "".join([f'{self.qmom[ist, jst, iat, isp]:15.8f}' for isp in range(self.aux.ndim)]) for iat in range(self.aux.nat)])
    ...
    def print_init(self, qm, mm, restart):
        ...
        if (self.verbosity >= 1):
            ...
            dynamics_step_info += "\n" + DEBUG1
        #if (self.verbosity >= 2):
        #    DEBUG2 = f" #DEBUG2{'STEP':>6s}{'SUM P_i':>13s}{'P':>7s}"
        #    dynamics_step_info += "\n" + DEBUG2
    ...
    def print_step(self, istep):
        ...
        #if (self.verbosity >= 2):
        #    P = self.mol.vel[0, 0] * self.mol.mass[0]
        #    sum_Pi = 0.0
        #    for ist in range(self.mol.nst):
        #        sum_Pi += self.aux.vel[ist, 0, 0] * self.mol.mass[0] * self.mol.rho.real[ist, ist]
        #    DEBUG2 = f" DEBUG1{istep + 1:>7d} {P:15.8f} {sum_Pi:15.8}"
