...
from build.el_propagator_isc_xf import el_run
...
#                raise ValueError (f"( {self.md_type}.{call_name()} ) {error_message} ( {error_vars} )")
    ...
    def hop_check(self, bo_list):
        ...
        for ist in range(self.mol.nst):
            ...
            if (self.rand > self.acc_prob[ist] and self.rand <= self.acc_prob[ist + 1]):
                self.l_hop = True
                if (self.mol.states[self.rstate].mult != self.mol.states[ist].mult):
                    self.l_isc = True
                self.rstate = ist
    ...
    def evaluate_hop(self, bo_list):
        ...
        if (self.l_hop):
            ...
            if ((self.l_isc or self.hop_rescale == "energy") and self.mol.ekin_qm < eps):
                self.l_reject = True
            ...
            if (self.l_reject):
                ...
                if (self.l_isc or (self.hop_rescale == "energy") or (det < 0. and self.hop_rescale == "augment")):
                    if (det < 0.):
            ...
            if (not (self.hop_reject == "keep" and self.l_reject)):
                if (self.l_isc or (self.hop_rescale == "energy")):
